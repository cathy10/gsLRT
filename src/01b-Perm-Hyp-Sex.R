...
Lambdas <- plyr::aaply(1:ncol(G), 1, function(j){
  ...
  gsHyp::lr_stats(y = G[, j], X = X, z = Z, interaction_term = "sex",
                  stat = "bartlett")
...
##
ES_df <- gsHyp::gsHyp(Z, X, G, interaction_term = "sex", gene_sets = gene_sets, n_perm = 2000)
## Make figures
gsHyp::plot_results(ES_df, Lambdas, gene_sets)
ggplot2::ggsave("results/figures/perm_hyp_sex.jpeg", height = 10, width = 5)
## Write tables
ES_df %>%
  ...
  plyr::arrange(p) %>%
  readr::write_tsv("results/tables/perm_hyp_sex.txt")
